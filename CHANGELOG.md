...
- 同步项目发布版本、release manifest 默认版本、goalcli 治理版本、API 文档、Harness final gate 和 release preflight 示例到 `v1.1.0`。
- 以次版本发布口径重新固定 Redis L2 公共写入面与 release Evidence，保持 GitHub Release、manifest 和公开文档版本一致。
...
- 同步项目发布版本、release manifest 默认版本、goalcli 治理版本、API 文档和 release preflight 示例到 `v1.0.4`。
- 重新发布 Redis L2 公共写入面与 release Evidence 口径，保持 Auto Patch Release、manifest 和公开文档版本一致。
...
- 本版本不改变 `pkg/redisx` 公共 API 形状；`Version` 元数据同步到 `v0.4.13`。
...
- 本版本不改变 `pkg/redisx` 公共 API 形状；`Version` 元数据同步到 `v0.4.7`。
...
- 本版本不改变 `pkg/redisx` 公共 API 形状；`Version` 元数据同步到 `v0.4.6`。
...
- 本版本不改变 `pkg/redisx` 公共 API，仅更新版本元数据、CI 治理和文档链接。
...
- 本版本不改变 `pkg/redisx` 公共 API，仅收紧 downstream debt 和 release Evidence 治理。
...
- 本版本不改变 `pkg/redisx` 公共 API，仅修复 release Evidence/generator 交付口径。

## v0.4.1 - 2026-06-03

### 治理

- 对齐 release 版本口径到 `v0.4.1`，同步 `redisx.Version`、release manifest 默认版本、preflight 命令示例和 Harness 记录。

### 兼容性

- 本版本仅更新发布治理与版本元数据，不改变 `pkg/redisx` 公共 API 行为。
...
- 本版本不改变 `pkg/redisx` 公共 API，仅收紧发布 Evidence 校验与文档门禁。
...
- 本版本仅包含治理和文档更新，不改变 `pkg/redisx` 公共 API。
...
- 发布文档、Agent 运行时文档和标准文档统一到 `redisx` 命名与 release gate 口径。
