...
func TestBuildDockerEvidenceReturnsDefaultsWhenEnvUnset(t *testing.T) {
	...
	if got.ToolchainImage != "redisx-toolchain:local" {
		t.Fatalf("docker.toolchain_image = %q, want redisx-toolchain:local", got.ToolchainImage)
	}
	...
	if got.RuntimeImage != "redisx-goalcli-runtime:local" {
		t.Fatalf("docker.runtime_image = %q, want redisx-goalcli-runtime:local", got.RuntimeImage)
	}
