...
func runVersion(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	return emitReport(stdout, "version", "passed", []string{"redisx release " + projectReleaseVersion, "goalcli governance runtime " + governanceRuntimeVersion, "goalcli governance CLI available"}, nil)
}
...
var contextProfileGates = map[string][]string{
	...
	"release":  {"context-full", "integration", "dependency-check", "standard-impact-check", "coverage-check", "score-check", "debt-evidence", "evidence", "release-evidence-hash", "release-evidence-check", "release-evidence-checksum-check"},
}
...
func runContextProfileCheck(command string, args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if makefile, err := os.ReadFile("Makefile"); err == nil {
		...
		appendMakefileTargetDependencyGaps(makefileText, "context-release", []string{"require-gowork-off", "context-full", "integration", "dependency-check", "standard-impact-check", "coverage-check", "score-check", "debt-evidence"}, []string{"context-standard", "release-check", "release-final-check"}, &gaps)
...
func appendAgentIndexGaps(path string, gaps *[]string) {
	...
	text := string(content)
	for _, needle := range []string{"schema_version:", "module: redisx", "control_plane:", "files:"} {
		if !strings.Contains(text, needle) {
			*gaps = append(*gaps, path+" missing "+needle)
		}
	}
