...
将 `redisx` 升级为 Full Goal Runtime v3.1 的统一标准、Go 参考模板、Generator、Harness 和 Evidence Runtime 仓库。

## 成功条件

- `README.md`、`docs/standard/`、ADR 和 `.agent/` 不再把旧 `redisx` / `foundationx` 当作主身份或默认下游。
- 旧名只用于迁移文档语境。
- 默认下游集成目标为 `kernel`；目标生成库矩阵覆盖 `configx`、`observex`、`testkitx`、`postgresx`、`redisx`、`kafkax`、`natsx`、`taosx`、`ossx`、`clickhousex`。
- `/home/k8s/secrets/env/*` 只属于调用方部署路径，不能被 `redisx`、`kernel` 或生成库读取、提交、记录到日志或写入 Evidence/manifest/PR。
