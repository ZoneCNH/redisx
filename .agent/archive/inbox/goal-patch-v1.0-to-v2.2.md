...
运行时控制平面
自动化系统
goalcli / harness / redisx / kernel / x.go
```
...
# 22. redisx / kernel / x.go 专用规则

## RULE-XSTACK-001：redisx 是标准源
...
tasks:
  - task_id: TASK-GOAL-20260603-001-001
    ...
    worktree: ~/code/.worktrees/redisx/GOAL-20260603-001/TASK-001
    status: READY
    evidence:
...
```text
~/code/.worktrees/redisx/GOAL-20260603-001/TASK-GOAL-20260603-001-001
```
...
## RULE-DOWNSTREAM-002：每次 redisx 规则更新必须说明下游影响
...
goal_id: GOAL-20260603-001
task_id: TASK-GOAL-20260603-001-001
worktree: /home/zone/code/.worktrees/redisx/GOAL-20260603-001/TASK-001
...
## RULE-DOWNSTREAM-SYNC-001：redisx 更新后必须生成同步任务
...
## RULE-FACTORY-001：redisx 的最终输出不是仓库，而是工厂能力
...
## RULE-DOWNSTREAM-CONTRACT-001：redisx 必须发布下游契约
...
```bash
goalcli bootstrap repo --profile redisx
```
...
| full       | redisx | standard + retro/adoption/audit/dashboard |
...
## RULE-PROMOTE-001：redisx 新规则不能直接推全量下游
...
## RULE-COMPAT-GUARD-001：下游兼容性失败不得阻断 redisx 内部发布
...
> **Goal Runtime v1.7 的核心已经足够完整。下一步不应继续堆规则，而应把 v1.0-v1.7 合并成一个可执行规范，并拆成 goalcli v0.1.0、redisx `.agent`、Harness Gates、CI Workflows、下游 adoption 五条落地线。**
...
> 把 redisx 从“规则文档集合”升级为“可机器裁判的 Goal Runtime 标准源”。
...
15. .agent/docs/templates/evidence-template.md
16. .agent/docs/templates/release-manifest-template.md
```
...
git worktree add \
  ~/code/.worktrees/redisx/GOAL-20260603-001/TASK-001 \
  -b goal/GOAL-20260603-001/TASK-001 \
  origin/main

cd ~/code/.worktrees/redisx/GOAL-20260603-001/TASK-001
...
```bash
git worktree remove ~/code/.worktrees/redisx/GOAL-20260603-001/TASK-001
git worktree prune
```
...
redisx 自证通过后，下一步不是全量推广，而是先拿 `kernel` 做试点。

## 目标

```text
验证 redisx Goal Runtime 是否能被 L0 内核库采用。
```
...
import (
	...
	"github.com/ZoneCNH/redisx/tools/goalcli/internal/config"
	"github.com/ZoneCNH/redisx/tools/goalcli/internal/gitutil"
	"github.com/ZoneCNH/redisx/tools/goalcli/internal/report"
)
...
import (
	...
	"github.com/ZoneCNH/redisx/tools/goalcli/internal/gitutil"
	"github.com/ZoneCNH/redisx/tools/goalcli/internal/report"
	"github.com/ZoneCNH/redisx/tools/goalcli/internal/worktree"
)
...
```go
module github.com/ZoneCNH/redisx/tools/goalcli
...
```go
"github.com/ZoneCNH/redisx/tools/goalcli/internal/secret"
```
