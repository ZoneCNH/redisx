...
if [[ "$package_name" != "redisx" && -e "$repo_dir/pkg/redisx" ]]; then
  echo "ERROR: stale pkg/redisx directory still exists" >&2
  exit 1
fi
...
if [[ "$package_name" != "redisx" ]]; then
  scan_fixed "pkg/redisx" "package directory reference"
  scan_fixed "redisx_" "metrics prefix"
  scan_fixed "Redisx" "title-case package name"
  scan_fixed "REDISX" "upper-case package name"
  scan_regex '\bredisx\b' "package name"
fi
