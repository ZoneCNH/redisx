...
本仓库是 Go 1.23 基础库标准与交付运行时，模块路径为 `github.com/ZoneCNH/redisx`，承担五类职责：**Standard Source**、**Go Reference Template**、**Generator**、**Harness** 和 **Evidence Runtime**。旧名 `redisx` / `foundationx` 仅允许出现在迁移文档中。

## 项目结构与模块组织

- `pkg/redisx`：公共 API 参考实现（config、client、health、errors、metrics、options、version），渲染后移到 `pkg/<package-name>`。
...
```bash
go test ./pkg/redisx/ -run TestConfigValidate
...
使用标准 Go 风格：交给 `gofmt` 处理缩进，包名保持简短，导出标识符要清晰表达用途。模板占位符 `redisx`、`github.com/ZoneCNH/redisx`、`redisx` 必须在代码和文档中保持一致。公共库能力放入 `pkg/redisx`，私有辅助逻辑放入 `internal/`。golangci-lint 启用的 linter 见 `.golangci.yml`。
