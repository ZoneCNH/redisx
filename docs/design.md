...
`redisx` 是 [`https://github.com/ZoneCNH/redisx`](https://github.com/ZoneCNH/redisx) 的统一标准、Go 参考模板、generator、Harness 和 Evidence Runtime 仓库。生成的库是独立 Go module。公共 API 位于 `pkg/redisx`，内部辅助代码位于 `internal/`，contracts 位于 `contracts/`，运行 Evidence 位于 `release/manifest/`。`scripts/render_template.sh` 是模板到具体基础库的唯一内置渲染入口。
