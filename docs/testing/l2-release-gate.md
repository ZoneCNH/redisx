...
- `L2-T2` 增加 integration evidence；当 adapter manifest 声明 persistence capability/profile 时，同时要求 provider recovery evidence。
- `L2-T3` 增加 chaos、benchmark 和 adoption evidence，并允许 release。
- `L2-T4` 增加 retrospective evidence，并允许 factory-grade claims。

## Gate behavior

当 required profiles、evidence paths、selected pack reports 或 compatibility notes 缺失时，gates 必须 fail closed。声明 persistence profile 但缺少 recovery report 时，必须视为 hard blocker。`xlibgate` 根据此 registry 裁决 evidence；`xlib-standard` 只定义 release semantics 和 template targets。
