...
5. 按顺序运行 profile 阶段：`l2-contract`、`l2-integration`、`l2-persistence`、`l2-chaos`、`l2-benchmark` 和 `l2-adoption`。
...
- `integration`、`persistence`、`chaos` 和 `benchmark` 证明 downstream service 行为，必须记录为 evidence，不得编码进标准源仓库。
- `adoption` 和 `retrospective` 证明 rollout safety、compatibility notes 和发布后复盘。

## Pass criteria

只有当每个已声明 capability 都映射到 registry pack、每个必需 profile 都有机器可读 evidence，且 `xlibgate` 确认请求的 release level 时，downstream adapter 才算 L2-ready。声明 `persist` 或 `persistence` profile 的 adapter 还必须提供 provider restart/recovery evidence。缺失 evidence 是 blocker，不能通过降低标准绕过。
