...
发布流程必须证明源码、contracts、依赖和 gate 状态一致。`redisx` 的 release 标准同时约束生成基础库；旧 `redisx` 仅作为迁移兼容名记录。
...
GOWORK=off make release-preflight VERSION=v1.1.2
```
