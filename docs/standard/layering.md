...
`redisx` 同时是 Standard 规则的独立来源和 Go 基础库模板中的实现仓库。旧 `redisx` 名称只用于迁移说明，不再表示独立主角色。

| 层级 | 示例 | 职责 | 禁止 |
| --- | --- | --- | --- |
| Standard/Runtime | `redisx` | 标准文本、参考模板、generator、Harness、Evidence、Goal Runtime | 真实基础设施 runtime、业务语义、生产密钥 |
...
- `redisx` 产生标准、模板和 gate，不依赖生成库或 `x.go`。
