...
本文件约束 `redisx`、生成基础库、`kernel` 和 `x.go` 的模块边界。边界违规必须在 PR、review 或 release gate 中阻断。旧名 `redisx` 只保留为迁移兼容名，不再是主仓库角色。

## `redisx` 允许内容

- P0 标准文档与版本说明。
- 仓库角色、分层、模块边界、下游兼容性规则和 module path 规则。
- Go 参考模板 `pkg/redisx`、contracts schema、metrics contract 和测试夹具。
...
- 把旧 `redisx` / `foundationx` 叙事作为主身份；旧名只能出现在迁移文档语境。
