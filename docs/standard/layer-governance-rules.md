...
本文定义 `redisx -> L0 -> L1 -> L2 -> L3` 的规则、管理边界、约束限制和后续更新迭代流程。若本文与更高权威文件冲突，以 `CONSTITUTION.md`、`docs/standard/module-boundary.md`、`.agent/harness/harness.yaml` 和 release Evidence gate 为准。决策背景见 [ADR-20260604-001](../adr/ADR-20260604-001-layer-governance.md)。

## 分层边界

| 层级 | 仓库/库 | 可见性 | 职责 |
| --- | --- | --- | --- |
| Standard/Runtime | `redisx` | 可公开 | 标准、模板、generator、Harness、Evidence 和治理规则 |
...
- `redisx` 不依赖 `kernel`、L1/L2 生成库或 `x.go`。
- L0 不依赖 L1/L2 或 `x.go`。
...
- 采纳状态：`.agent/registries/downstream-adoption-status.yaml`
- 机器 gate：`.agent/harness/harness.yaml`、`cmd/goalcli`
...
1. 在 `redisx` 提出标准变更、ADR 或 Issue，说明影响层级和是否触发下游同步。
2. 更新 `docs/standard/**`、contracts、模板、Harness 或 `goalcli`，并保持文档默认中文叙述。
3. 更新 `docs/downstream-matrix.md`、`docs/downstream-sync-policy.md` 和 `.agent/registries/downstream-adoption-status.yaml` 中受影响库的状态。
4. 使用 `GOWORK=off` 运行对应 gate，至少覆盖 `docs-check`；触达发布、contracts、security 或 generator 时运行更高等级 gate。
5. 发布 `redisx` tag 后，按 L0 -> L1 -> L2 顺序同步基础库。
